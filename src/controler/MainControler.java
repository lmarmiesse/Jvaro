...
public class MainControler extends Observable {
	...
	public List<Object[]> getReactions() {
		...
		for (Reaction reac : reactions) {
			listeObj.add(new Object[] { reac.getName(), reac.isReversible(), reac.getStringReaction() });
	...
	public List<Object[]> getReactants(String reacName) {
		return project.getNetwork().getReaction(reacName).getObjetReactantsTable();
	}
	...
	public List<Object[]> getMetabos() {
		...
		for (Metabolite metab : metabolites) {
			listeObj.add(new Object[] { metab.getName(), metab.isInternal(), metab.getDescription() });
	...
	public void loadNetwork(File[] file) throws ClassNotFoundException, IOException {
		...
		new Thread() {
			public void run() {
				...
				running.add(new JLabel("Loading file ..."), BorderLayout.PAGE_START);
	...
	public void saveNetworkAs(File file) throws IOException, SBMLException, XMLStreamException {
